...
namespace TxFlow.DebugAdapter.Protocol
{
	...
	public class Thread
	{
		...
		public Thread(int id, string name = null) {
			this.id = id;
			if (name == null || name.Length == 0) {
				this.name = string.Format("Workflow #{0}", id);
			} else {
				this.name = name;
			}
		}
	...
	public class StackTraceResponseBody : ResponseBody
	{
		...
		public StackTraceResponseBody(IEnumerable<StackFrame> frames, int total = -1) {
			stackFrames = frames.ToArray<StackFrame>();
			totalFrames = total < 0 ? frames.Count() : total;
		}
	}
	...
	public class VariablesResponseBody : ResponseBody
	{
		...
		public VariablesResponseBody(IEnumerable<Variable> vars) {
	...
	public class ThreadsResponseBody : ResponseBody
	{
		...
		public ThreadsResponseBody(IEnumerable<Thread> ths) {
			...
			try {
				...
				case "initialize":
					if (args.linesStartAt1 != null) {
                            //DebugInformationProvider.Instance.ClientLinesStartAt1 = (bool)args.linesStartAt1;
					}
					var pathFormat = (string)args.pathFormat;
					if (pathFormat != null) {
						switch (pathFormat) {
						case "uri":
                            //DebugInformationProvider.Instance.ClientPathsAreURI = true;
							break;
						case "path":
                            //DebugInformationProvider.Instance.ClientPathsAreURI = false;
                            break;
						default:
